...
! 2016-03-03  Robert Grumbine  Updates for better F90 compliance
!
! USAGE:
!   INPUT FILES:
...
!D      CALL W3TAGB('SSMIBUFR',1999,0248,0070,'NP21   ')
       OPEN (FILE=iunt, FORM="UNFORMATTED", STATUS="OLD")
       OPEN (FILE=ount, FORM="UNFORMATTED", STATUS="NEW")
     ...
     1            brt858(index, index2, index3)
           ...
           IF (jret .GT. 1) THEN
             ...
             iscan = XIDENT(9)
             isec = XIDENT(7)
!D             creturns = shortout(SATNO, iy,im,id,ihr,imins,isec,iscan)
!D             IF (count .LT. 100) THEN
               WRITE (*,*) SATNO, iy,im,id,ihr,imins,isec,iscan
!D             ENDIF
            ...
            DO kscan = 1,iret
              ...
              tmp6 = brt(2,6,kscan)
              tmp7 = brt(2,7,kscan)
              WRITE (*,*) kscan,xlat,xlon,sftg,posn,
...
!             Now write out the hires info:
              DO k = 1, 3
                ...
                WRITE (*,*) kwrit, xlat, xlon, posn, tmp1, tmp2
