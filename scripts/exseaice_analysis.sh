...
#-----------------------------------------------------------
# Produce the VIIRS-based L3 global grid from the ascii files generated by exseaice_viirs
#VIIRS export pgm=seaice_viirscomp
#VIIRS . prep_step
#VIIRS startmsg
#VIIRS time $EXECseaice_analysis/seaice_viirscomp ir_latlon.$PDY $FGLAND5MIN $COMOUT/viirs.*.* 
#VIIRS export err=$?;err_chk
#VIIRS touch ir_latlon.$PDY

# Save point for initial L2, L3 analyses:
#-----------------------------------------------------------
...
#"" add viirs to global_blend 9 Oct 2025
#---------------------------------------------------------
...
then
  ...
  # Produce passive microwave L3 global
  startmsg
  . prep_step
  ...
  startmsg
  $EXECseaice_analysis/seaice_icegrid $COMINm1/seaice.${cycle}.fill5min final_latlon.$PDY \
...
$EXECseaice_analysis/seaice_edge fill5min.$PDY $FGLAND5MIN seaice_edge_${PDY}.kml > seaice_edge_${PDY}.txt
  ...
  then
    ...
    cp final_latlon.$PDY ${COMOUT}/seaice.${cycle}.latlon_final
