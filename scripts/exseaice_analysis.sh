...
if [ $SENDCOM = "YES" ]
then
  cp wmonorthpsg.${PDY}.grb ${COMOUTwmo}/seaice.t${cyc}z.wmonorthpsg.${job}
  cp wmosouthpsg.${PDY}.grb ${COMOUTwmo}/seaice.t${cyc}z.wmosouthpsg.${job}
  if [ $SENDDBN = YES ]
  then
    $DBNROOT/bin/dbn_alert GRIB_LOW $NET $job  ${COMOUTwmo}/seaice.t${cyc}z.wmonorthpsg.${job}
    $DBNROOT/bin/dbn_alert GRIB_LOW $NET $job  ${COMOUTwmo}/seaice.t${cyc}z.wmosouthpsg.${job}
  fi
fi
...
#####################################################################
  ...
  if [ $SENDCOM = "YES" ]
  then
    cp wmoglobice.${PDY}.grb ${COMOUTwmo}/seaice.t${cyc}z.wmoglobice.${job}
  ...
  if [ $SENDDBN = "YES" ]
  then
    $DBNROOT/bin/dbn_alert GRIB_LOW $NET $job ${COMOUTwmo}/seaice.t${cyc}z.wmoglobice.${job}
    $DBNROOT/bin/dbn_alert GRIB_LOW $NET $job ${COMOUTwmo}/grib2_awips_seaice.t${cyc}z.5min.grb
  fi
